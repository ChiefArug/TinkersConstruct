...
import slimeknights.tconstruct.library.tools.capability.inventory.ItemBlockPlacerCapability;
...
public class ExchangingModifier extends NoLevelsModifier implements RemoveBlockModifierHook {
  ...
  public Boolean removeBlock(IToolStackView tool, ModifierEntry modifier, ToolHarvestContext context) {
    ...
    ItemBlockPlacerCapability blockPlacer = ItemBlockPlacerCapability.getBlockProvider(offhand);
    if (blockPlacer == null) {
      return null;
    }
    Block block = blockPlacer.getBlock();
    if (block == null) {
      return null;
    }
    ...
    BlockHitResult hitResult = Util.createTraceResult(pos, context.getSideHit().getOpposite(), true);
    // swap the block, it never goes to air so things like torches will remain
    InteractionResult success = blockPlacer.place(world, player, InteractionHand.OFF_HAND, hitResult, true);
